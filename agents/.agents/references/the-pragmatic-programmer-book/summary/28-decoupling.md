...
- Try to limit each access to one method or property hop; replace longer chains, including chains split across intermediate variables, with intention-revealing operations on responsible objects.
...
- Chain calls only across genuinely stable interfaces, such as language libraries, and treat application and third-party interfaces as likely to change.
- Prefer pipelines with explicit, compatible data formats over traversal through hidden object structure.
- Avoid global mutable data, including disguised globals in singletons and modules.
- Wrap necessary global data behind an API that you control, while recognizing that methods do not make a single shared data set less global.
