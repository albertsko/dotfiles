...
- Configure a concise prompt with useful context such as the current directory, version-control status, and time. Avoid adding information that does not affect your next action.
...
- Evaluate other shells when the current shell cannot express a needed workflow, and preserve your shell configuration so you can reproduce a productive environment on another system.
