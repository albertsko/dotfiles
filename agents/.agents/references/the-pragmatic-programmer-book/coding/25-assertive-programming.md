...
Treat every “impossible” state as an invariant to enforce, because production exposes code to untested inputs, resource failures, and environmental conditions. Use assertions to detect programmer errors close to the violated assumption and report enough context to diagnose them, but handle expected input, system, and operational failures through normal error paths. Keep assertion expressions free of side effects, and leave assertions enabled in production unless measurement shows that a specific expensive check must become optional.
...
- Release resources through a safe failure path when an assertion stops execution, and do not let cleanup code depend on the invalid data that triggered the failure.
