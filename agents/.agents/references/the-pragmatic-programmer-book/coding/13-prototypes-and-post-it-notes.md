...
- Choose the cheapest medium that preserves the behavior under investigation. Use Post-it notes for workflows, cards or a whiteboard for component relationships, a clickable mock-up for interface interactions, or a short script for combining existing components.
...
- Accept limited correctness, completeness, error handling, documentation, and polish, but constrain the experiment so those omissions cannot invalidate its result.
- Evaluate architecture by checking component responsibilities, collaborations, coupling, duplicated work, interface constraints, and whether each component can access required data at the exact time it needs it.
- Time-box the prototype, record the lessons and decisions it produces, then discard the implementation rather than treating prototype code as a production foundation.
