...
- Prefer domain-specific roles over generic labels: use `buyer` or `customer` instead of `user` when the distinction affects behavior.
...
- Establish a shared project vocabulary, record specialized terms in a glossary, and reinforce the same terms through frequent code review and collaboration.
...
- Rename an element as soon as its meaning changes or its name becomes confusing, and maintain tests and low coupling so renames remain safe and routine.
