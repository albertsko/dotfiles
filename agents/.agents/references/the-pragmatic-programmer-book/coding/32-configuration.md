...
- Put shared or runtime-changeable configuration behind a service API. Apply authentication and access control so each application can read only the values it needs, and provide a specialized user interface when operators or customers maintain those values.
