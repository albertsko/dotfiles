...
- Start an internal domain language with ordinary functions and objects. Use host-language features such as loops, functions, and composition to generate repetitive domain declarations, tests, or configurations.
- Accept the host language's syntax and semantics as constraints on an internal domain language. Add macros or metaprogramming only when simpler constructs cannot express the domain clearly and the added flexibility justifies the complexity.
- Prefer an established external format when its syntax fits the problem. If it does not, prefer an internal language and consider a custom external language only when application users will write it and independent syntax provides enough value to justify a parser.
...
- Look for domain requirements that a compiler or translator can turn into repetitive application code, and reuse language infrastructure across projects when doing so produces net savings.
