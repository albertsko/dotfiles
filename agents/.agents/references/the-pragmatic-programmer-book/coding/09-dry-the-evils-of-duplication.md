...
- Define each internal interface in a neutral, central specification, store it where teams can share it, and generate clients, documentation, mocks, and functional tests from it.
- Import a formal external interface specification when one exists. When none exists, create, publish, and maintain one as the authoritative representation.
...
- Make reuse easier than reimplementation: search for existing utilities before adding shared functionality, keep reusable code in a discoverable central location, assign someone to facilitate knowledge exchange when useful, and surface overlapping work through frequent communication, code reviews, and reading teammates' code.
