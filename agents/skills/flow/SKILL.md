...
Apply recommendations through controls exposed by the active session. If those controls are unavailable, retain the current settings. Report a mismatch only when it materially affects the task.
...
- Keep task notes and plans in this directory. Reuse it across stages of the same task.
...
- Delegate only when available, permitted, and worth the coordination cost. Otherwise, work in the current session.
- Give each worker the relevant context, scope, constraints, completion checks, and expected output.
- Run workers in parallel only when their tasks have no sequential dependencies.
...
- Read the relevant code, instructions, and evidence. Establish the requested outcome, constraints, and observable completion checks.
...
For long tasks, keep durable notes of decisions, completed steps with evidence, blockers, and the next action. Update the notes as work progresses. Before resuming after an interruption or handoff, read the notes and reconcile them with the repository state to avoid repeating completed work.
...
- When an approach fails, change the hypothesis, evidence, or method before retrying. Revisit the plan when new evidence invalidates it.
...
- Run checks appropriate to the final state. For behavior fixes, demonstrate the failure and its correction when feasible.
